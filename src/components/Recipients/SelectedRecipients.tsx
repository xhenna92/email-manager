...
const SelectedRecipients = (props: SelectedRecipientProps) => {
    ...
    const groupSelectedEmailData = () => {
        ...
        return { "company recipients": companyRecipients, "email recipients": emailRecipients };
    }
    ...
    const emailDisplay = (e: string) => {
        return <Box key={`${e}-selected`} as="span" flex='1' textAlign='left' justifyContent="space-between" display='flex' padding={'0.5rem 1rem'}>
            <span>{e}</span>
    ...
    const emptyState = (k: string) => {
        return <Box as="span" flex='1' textAlign='left' justifyContent="space-between" display='flex' padding={'0.5rem 1rem'}>
    ...
    return <Layout title='Selected Recipients'>
                            ...
                            return <AccordionItem key={key}>
                                ...
                                <AccordionPanel pb={4}>
                                    {items.length > 0 ? items.map((company) => {
                                        return <Box key={`${company}-company-selected`} as="span" flex='1' textAlign='left' justifyContent="space-between" display='flex' padding={'0.5rem 1rem'}>
                                            <span>{company}</span>
                                            <CloseButton size="sm" onClick={() => { deselectAllEmailsForCompany(company)}} />
                                        </Box>;
                                    }) : emptyState('company')
                                    }
                                </AccordionPanel>
